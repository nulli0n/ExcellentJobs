...
public class GrindWorkstationListener extends AbstractListener<JobsPlugin> {

    private final GrindManager manager;

    public GrindWorkstationListener(@NonNull JobsPlugin plugin, GrindManager manager) {
        super(plugin);
    ...
    public void onWorkstationPlace(BlockPlaceEvent event) {
        ...
        this.modifyStandNextTick(workstation, station -> this.manager.setWorkstationOwnerId(station, player
            .getUniqueId()));
    }
    ...
    public void onWorkstationAutomaticInventoryMoveEvent(InventoryMoveItemEvent event) {
        ...
        this.modifyStandNextTick(workstation, station -> this.manager.setWorkstationMode(station,
    ...
    private void handleWorkstationInteraction(InventoryInteractEvent event) {
    ...
    private void modifyStandNextTick(Workstation workstation, Consumer<TileState> consumer) {
        this.plugin.runTask(() -> {
            if (!(workstation.getBackend().getLocation().getBlock().getState() instanceof TileState tickedStation))
                return;
