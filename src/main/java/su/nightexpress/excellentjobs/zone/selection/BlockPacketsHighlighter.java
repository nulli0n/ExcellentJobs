...
public class BlockPacketsHighlighter extends BlockHighlighter {
    ...
    public BlockPacketsHighlighter(JobsPlugin plugin) {
        super(plugin);
        this.manager = PacketEvents.getAPI().getPlayerManager();
    }

    @Override
    protected void spawnVisualBlock(int entityID, Player player, Location location,
                                    BlockData blockData, ChatColor color, float size) {
        ...
        WrapperPlayServerTeams.ScoreBoardTeamInfo info = new WrapperPlayServerTeams.ScoreBoardTeamInfo(
            Component.text(entityUID), Component.text(""), Component.text(
                ""), WrapperPlayServerTeams.NameTagVisibility.ALWAYS, WrapperPlayServerTeams.CollisionRule.ALWAYS, NamedTextColor.NAMES
                    .valueOr(color.name().toLowerCase(), NamedTextColor.WHITE), WrapperPlayServerTeams.OptionData.NONE
        );
        var teamPacket = new WrapperPlayServerTeams(entityUID, WrapperPlayServerTeams.TeamMode.CREATE, info, Lists
            .newList(entityUID));
    ...
    protected void destroyEntity(Player player, List<Integer> idList) {
        WrapperPlayServerDestroyEntities packet = new WrapperPlayServerDestroyEntities(idList.stream().mapToInt(i -> i)
            .toArray());
        this.manager.sendPacket(player, packet);
    }


    private WrapperPlayServerSpawnEntity createSpawnPacket(EntityType type, Location location,
        ...
        com.github.retrooper.packetevents.protocol.world.Location wrappedLocation = SpigotConversionUtil
